...
# ══════════════════════════════════════════════════════════════════════════════
# EXPORT BLOCK 2 – PER-(COMPANY × COMMODITY) BIAS-CORRECTED PREDICTIONS
# ──────────────────────────────────────────────────────────────────────────────
#
# PURPOSE
# -------
# The OLS-FE model predicts log-total carloads at the panel grain
# (one row per company × commodity × week).  When summed to weekly totals
# the aggregate fit is reasonable, but individual cells carry systematic
# directional errors (bias) because the Ridge penalty shrinks fixed-effect
# coefficients toward zero.
#
# This block adds an additive bias-correction layer computed entirely from
# the in-sample residuals, then produces a single flat CSV covering both:
#   (a) in-sample weeks  – raw OLS-FE fit alongside bias-corrected fit
#   (b) forecast horizon – OLS-FE extrapolation + bias correction applied
#
# Bias definition (cell-level, additive):
#   bias[company, commodity] = mean_t ( actual_total[t] − pred_total_raw[t] )
#                              averaged over all in-sample weeks
#
#   pred_total_bc = pred_total_raw + bias          (clipped to ≥ 0)
#
# Originated / Received split
# ---------------------------
# The OLS-FE model predicts *total* carloads only (log1p(originated+received)).
# To recover the component split we use each cell's in-sample mean share:
#   orig_share[company, commodity] = mean_t ( originated[t] / total[t] )
#
#   pred_originated_bc = pred_total_bc * orig_share
#   pred_received_bc   = pred_total_bc * (1 − orig_share)
#
# For in-sample rows the actual originated/received columns are preserved.
# For forecast rows (no actuals available) these are NaN.
#
# Bias stability diagnostics
# --------------------------
# A companion CSV (*_bias_table.csv) is written with per-cell bias statistics:
#   bias_mean, bias_std, bias_cv (= std/|mean|), n_obs, orig_share_mean
# This lets analysts flag cells where the bias estimate is noisy (high CV)
# and should therefore be applied with caution.
# ══════════════════════════════════════════════════════════════════════════════


def export_bias_corrected_panel_predictions(
    panel: pd.DataFrame,
    fe_model: "FixedEffectsOLS",
    ts: pd.DataFrame,
    horizon: int = FORECAST_H,
    bias_window_weeks: int = None,
    out_dir: str = OUTPUT_DIR,
    file_stem: str = "panel_bias_corrected",
    verbose: bool = True,
) -> pd.DataFrame:
    """
    Compute additive per-(company × commodity) bias corrections from OLS-FE
    in-sample residuals and export a flat CSV with both in-sample and
    forecast rows.

    Parameters
    ----------
    panel : pd.DataFrame
        Full panel data as returned by ``build_panel_data()`` (and optionally
        with a ``fe_pred`` column already attached).  Must contain columns:
        week, company, commodity_group, commodity_group_code,
        originated, received, total, week_num, week_of_year.
    fe_model : FixedEffectsOLS
        A FixedEffectsOLS instance that has already been fitted on the full
        in-sample panel.
    ts : pd.DataFrame
        Aggregate weekly time-series DataFrame (used only for date sequencing
        when building the forecast panel rows).
    horizon : int
        Number of weeks to forecast beyond the last observed week.
    bias_window_weeks : int or None
        If set, only the most-recent ``bias_window_weeks`` in-sample weeks are
        used to estimate the per-cell bias.  None (default) uses all in-sample
        observations — more stable for thin cells, less adaptive to drift.
    out_dir : str
        Directory where the CSV files are written.
    file_stem : str
        Base filename (without extension) for the two output files:
          - ``{file_stem}.csv``            – full row-level export
          - ``{file_stem}_bias_table.csv`` – per-cell bias statistics
    verbose : bool
        Print progress and summary statistics when True.

    Returns
    -------
    pd.DataFrame
        The full export DataFrame (in-sample + forecast rows) so callers
        can do further analysis without re-reading the CSV.

    Output columns
    --------------
    week                  – ISO week-start date
    company               – railway company identifier
    commodity_group       – commodity group label
    commodity_group_code  – commodity group code
    split                 – "in_sample" | "forecast"
    actual_originated     – observed originated carloads  (NaN for forecast)
    actual_received       – observed received carloads    (NaN for forecast)
    actual_total          – observed total carloads       (NaN for forecast)
    pred_total_raw        – raw OLS-FE predicted total (before bias correction)
    bias                  – additive correction = mean(actual − pred) per cell
    pred_total_bc         – bias-corrected total  = pred_total_raw + bias  (≥0)
    pred_originated_bc    – pred_total_bc × in-sample mean orig_share
    pred_received_bc      – pred_total_bc × in-sample mean recv_share
    orig_share            – in-sample mean originated fraction for this cell
    bias_cv               – bias coefficient of variation (std/|mean|); high
                            values flag cells where the bias estimate is noisy
    """

    os.makedirs(out_dir, exist_ok=True)

    # ── Step 1: Attach raw OLS-FE in-sample predictions ──────────────────────
    if verbose:
        print("[export_bias_corrected] Computing in-sample OLS-FE predictions …")

    panel = panel.copy()

    # Re-predict on the full panel (idempotent – works even if fe_pred already exists)
    panel["pred_total_raw"] = np.clip(fe_model.predict(panel), 0, None)

    # ── Step 2: Determine the bias estimation window ──────────────────────────
    all_weeks_sorted = sorted(panel["week"].unique())

    if bias_window_weeks is not None and bias_window_weeks < len(all_weeks_sorted):
        bias_weeks = set(all_weeks_sorted[-bias_window_weeks:])
        if verbose:
            print(f"  Bias window: last {bias_window_weeks} weeks "
                  f"({min(bias_weeks).date()} → {max(bias_weeks).date()})")
    else:
        bias_weeks = set(all_weeks_sorted)   # use all in-sample weeks
        if verbose:
            print(f"  Bias window: full in-sample period "
                  f"({min(bias_weeks).date()} → {max(bias_weeks).date()})")

    bias_panel = panel[panel["week"].isin(bias_weeks)].copy()

    # ── Step 3: Per-cell bias and share statistics ────────────────────────────
    #
    # bias_mean = mean( actual_total − pred_total_raw )
    # orig_share = mean( originated / total )   — clipped to avoid div/0
    # ──────────────────────────────────────────────────────────────────────────

    # NOTE: load_raw_data() renames "commodity_group_code" → "code" in the panel.
    # We group on the actual column names present in the DataFrame.
    KEY = ["company", "commodity_group", "code"]

    bias_panel["residual"]  = bias_panel["total"] - bias_panel["pred_total_raw"]
    bias_panel["orig_frac"] = np.where(
        bias_panel["total"] > 0,
        bias_panel["originated"] / bias_panel["total"],
        np.nan,
    )
    ...
    cell_stats = (
        ...
        .agg(
            ...
            orig_share     = ("orig_frac", "mean"),
            n_obs          = ("residual",  "count"),
        )
        .reset_index()
    )

    # Coefficient of variation for the bias estimate (flagging unreliable cells)
    cell_stats["bias_cv"] = (
        cell_stats["bias_std"].abs() /
        (cell_stats["bias_mean"].abs() + 1.0)   # +1 prevents Inf when bias≈0
    )

    # Fill NaN orig_share (cells with no actuals > 0) with the company-level mean
    company_orig_share = (
        cell_stats
        .groupby("company")["orig_share"]
        .transform(lambda s: s.fillna(s.mean()))
    )
    cell_stats["orig_share"] = cell_stats["orig_share"].fillna(company_orig_share)
    cell_stats["orig_share"] = cell_stats["orig_share"].fillna(0.5)  # global fallback

    if verbose:
        n_cells = len(cell_stats)
        n_noisy = (cell_stats["bias_cv"] > 1.0).sum()
        print(f"  Cells (company × commodity): {n_cells}")
        print(f"  Cells with bias_cv > 1.0 (noisy estimate): {n_noisy} "
              f"({100*n_noisy/n_cells:.1f}%) — consider wider bias window")
        print(f"  Bias range: [{cell_stats['bias_mean'].min():+,.0f}, "
              f"{cell_stats['bias_mean'].max():+,.0f}] carloads/week")

    # ── Step 4: Apply bias to in-sample rows ──────────────────────────────────
    if verbose:
        print("[export_bias_corrected] Assembling in-sample export rows …")

    in_sample = panel.merge(cell_stats[KEY + ["bias_mean", "orig_share", "bias_cv"]],
                            on=KEY, how="left")
    in_sample["bias"]             = in_sample["bias_mean"].fillna(0.0)
    in_sample["pred_total_bc"]    = np.clip(
        in_sample["pred_total_raw"] + in_sample["bias"], 0, None
    )
    in_sample["pred_originated_bc"] = in_sample["pred_total_bc"] * in_sample["orig_share"]
    in_sample["pred_received_bc"]   = (
        in_sample["pred_total_bc"] * (1.0 - in_sample["orig_share"])
    )
    in_sample["split"] = "in_sample"

    # Rename actuals columns for clarity
    in_sample = in_sample.rename(columns={
        "originated": "actual_originated",
        "received":   "actual_received",
        "total":      "actual_total",
    })

    # ── Step 5: Build forecast panel rows ─────────────────────────────────────
    if verbose:
        print(f"[export_bias_corrected] Generating {horizon}-week forecast panel …")

    last_week_num   = panel["week_num"].max()
    last_week_date  = ts["week"].iloc[-1]
    future_dates    = pd.date_range(
        last_week_date + pd.Timedelta(weeks=1),
        periods=horizon, freq="W-WED",
    )

    # Template: take the last observed week's panel rows as the structural template
    # (all company × commodity combinations present in the final week)
    template = panel[panel["week"] == panel["week"].max()].copy()
    ...
    for h, fdate in enumerate(future_dates, start=1):
        ...
        # Roll week_of_year using modular arithmetic (1-52 cycle)
        fp["week_of_year"] = ((fp["week_of_year"] - 1 + h) % 52) + 1
        fp["year"]         = fdate.year
        # Clear actuals — no ground truth for forecast horizon
        fp["originated"]   = np.nan
        fp["received"]     = np.nan
        fp["total"]        = np.nan
        fp["log_total"]    = np.nan
        forecast_rows.append(fp)

    forecast_panel = pd.concat(forecast_rows, ignore_index=True)

    # Ensure categorical dtypes match the fitted model
    forecast_panel["company"] = forecast_panel["company"].astype(
        panel["company"].dtype
    )
    forecast_panel["code"] = forecast_panel["code"].astype(
        panel["code"].dtype
    )

    # Raw OLS-FE predictions on the extended panel
    forecast_panel["pred_total_raw"] = np.clip(
        fe_model.predict(forecast_panel), 0, None
    )

    # Merge bias and share statistics
    forecast_panel = forecast_panel.merge(
        cell_stats[KEY + ["bias_mean", "orig_share", "bias_cv"]],
        on=KEY, how="left",
    )
    forecast_panel["bias"]               = forecast_panel["bias_mean"].fillna(0.0)
    forecast_panel["pred_total_bc"]      = np.clip(
        forecast_panel["pred_total_raw"] + forecast_panel["bias"], 0, None
    )
    forecast_panel["pred_originated_bc"] = (
        forecast_panel["pred_total_bc"] * forecast_panel["orig_share"]
    )
    forecast_panel["pred_received_bc"]   = (
        forecast_panel["pred_total_bc"] * (1.0 - forecast_panel["orig_share"])
    )
    forecast_panel["split"] = "forecast"

    # Rename actuals columns to match in_sample schema
    forecast_panel = forecast_panel.rename(columns={
        "originated": "actual_originated",
        "received":   "actual_received",
        "total":      "actual_total",
    })

    # ── Step 6: Combine and select final columns ───────────────────────────────
    # 'code' is the internal panel column (renamed from commodity_group_code
    # by load_raw_data).  Restore the descriptive name in the output CSV.
    EXPORT_COLS = [
        "week", "company", "commodity_group", "code",
        "split",
        "actual_originated", "actual_received", "actual_total",
        "pred_total_raw",
        "bias", "pred_total_bc",
        "pred_originated_bc", "pred_received_bc",
        "orig_share", "bias_cv",
    ]

    out = (
        pd.concat(
            [in_sample[EXPORT_COLS], forecast_panel[EXPORT_COLS]],
            ignore_index=True,
        )
        .sort_values(["company", "commodity_group", "week"])
        .reset_index(drop=True)
        .rename(columns={"code": "commodity_group_code"})  # restore readable name
    )

    # ── Step 7: Write CSVs ────────────────────────────────────────────────────
    main_path  = os.path.join(out_dir, f"{file_stem}.csv")
    bias_path  = os.path.join(out_dir, f"{file_stem}_bias_table.csv")

    out.to_csv(main_path, index=False)

    # Bias table: one row per cell with diagnostics
    bias_table_out = cell_stats.sort_values(
        "bias_mean", ascending=False, key=abs
    ).reset_index(drop=True)
    bias_table_out.to_csv(bias_path, index=False)

    if verbose:
        n_is   = (out["split"] == "in_sample").sum()
        n_fc   = (out["split"] == "forecast").sum()
        print(f"  In-sample rows : {n_is:,}")
        print(f"  Forecast rows  : {n_fc:,}")
        print(f"  ✓  Predictions → {main_path}")
        print(f"  ✓  Bias table  → {bias_path}")

        # Quick aggregate diagnostic: compare raw vs BC in-sample
        is_df = out[out["split"] == "in_sample"].copy()
        agg = is_df.groupby("week").agg(
            actual=("actual_total",   "sum"),
            raw   =("pred_total_raw", "sum"),
            bc    =("pred_total_bc",  "sum"),
        )
        rmse_raw = np.sqrt(np.mean((agg["actual"] - agg["raw"]) ** 2))
        rmse_bc  = np.sqrt(np.mean((agg["actual"] - agg["bc"])  ** 2))
        bias_raw = float((agg["actual"] - agg["raw"]).mean())
        bias_bc  = float((agg["actual"] - agg["bc"]).mean())
        print("\n  ── Aggregate weekly RMSE (in-sample) ──────────────────")
        print(f"     OLS-FE raw       : RMSE = {rmse_raw:>10,.0f}  |  "
              f"mean bias = {bias_raw:>+10,.0f}")
        print(f"     Bias-corrected   : RMSE = {rmse_bc:>10,.0f}  |  "
              f"mean bias = {bias_bc:>+10,.0f}")
...
#%%
    ...
    # ── 9.8  Per-(company × commodity) bias-corrected export ──────
    print("\n[8/8] Exporting bias-corrected panel predictions…")
    #
    # Full in-sample window bias: stable estimate across all years.
    # Swap bias_window_weeks=52 for a rolling 1-year recency bias instead.
    export_bias_corrected_panel_predictions(
        panel        = panel,
        fe_model     = fe_full,
        ts           = ts,
        horizon      = FORECAST_H,
        bias_window_weeks = None,   # None → full in-sample; set e.g. 52 for 1-yr window
        out_dir      = OUTPUT_DIR,
        file_stem    = "panel_bias_corrected",
        verbose      = True,
    )
